...
//	$db->SetFetchMode(ADODB_FETCH_ASSOC); 
	...
	if($_POST["opcion"]=="GUARDA_TEPORAL"){
		print "tupapa";
		limpiarTablas($dbAcs);	//Limpia las tablas temporales
		...
		//		$arrTodo = array();
				...
				if ($rsGroup3) {
					while ($row  = $rsGroup3->fetchRow()) {
						...
						$strsqlMovtos.="(Movto, Cuenta, cuenta_txt,";
						...
						$strsqlMovtos.="UserActualizacion, Cuentanombre) ";
						
						$strsqlMovtos.="VALUES ";
						$strsqlMovtos.="($iNumMovto, $lCuenta, $sCuenta,";   //se cambio $lCuenta por $Cuenta
						...
						$strsqlMovtos.="$nDeposito, '".substr($row["cuenta_contpaq"],3)."', ";
						...
						print "\n$strsqlMovtos\n";
						$result = $dbAcs -> execute($strsqlMovtos);
		...
		//						array_push($arrPagos,$pago);	
								...
								$result2 = $dbAcs -> execute($strsqlMovPolCh);
								//odbc_prepare($pxVirtual2,$strsqlMovPolCh); if (odbc_execute($result2)) {}else {echo "Eres Un Error-MovPolCh\n"; } //odbc_close($pxVirtual2);
		...
		//				array_push($banco["group3"],$row);
				...
				if ($strsqlUtilidadPerdida!="") {
					$result3 = $dbAcs -> execute($strsqlMovPolCh);
					//odbc_prepare($pxVirtual2,$strsqlUtilidadPerdida); if (odbc_execute($result3)) {}else {echo "Eres Un Error-UtilidadPerdida\n"; } //odbc_close($pxVirtual2);
				}
				...
				$result4 = $dbAcs -> execute($strsqlMovPrincipal);
				//odbc_prepare($pxVirtual2,$strsqlMovPrincipal); if (odbc_execute($result4)) {}else {echo "Eres Un Error-MovPrincipal\n"; } //odbc_close($pxVirtual2);
				...
				$result5 = $dbAcs -> execute($strsqlPolizaPago);
				//odbc_prepare($pxVirtual2,$strsqlPolizaPago); if (odbc_execute($result5)) {}else {echo "Eres Un Error-PolizaPago\n"; } //odbc_close($pxVirtual2);
				
				//Agrega Deposito
				$strsqlDeposito = "INSERT INTO Depositos (CIdDeposito, cIDCuenta, CImporte, CConceptoGral, CTipoIngreso, CFechaDeposito, CFechaAplicacion, CEstadoConciliado, CEstadoContable, CIDPoliza, CTSAlta, CTSUltimaActualizacion, CUserAlta, CUserActualizacion, cuenta_txt)";
				$strsqlDeposito.= "VALUES ($nDeposito , $lCuenta ,". round($dSubtotalBancos,2).", 'Depositos al $fechaDeposito',-1, '$fechaDeposito', '$fechaDeposito', 0, 1, $nIdPoliza, now(), now(), 'SUPERVISOR', 'SUPERVISOR', $sCuenta)";
				
				$granQueryDeposito.= $strsqlDeposito."\n";
				$result6 = $dbAcs -> execute($strsqlDeposito);
				//odbc_prepare($pxVirtual2,$strsqlDeposito); if (odbc_execute($result6)) {}else {echo "Eres Un Error-Deposito\n"; } 
	...
	function limpiarTablas(&$dbAcs){
		...
		$rs1 = $dbAcs -> execute($borrarMovtos);
		$rs2 = $dbAcs -> execute($borrarMovPolCh);
		$rs3 = $dbAcs -> execute($borrarPolchqpW);
		$rs4 = $dbAcs -> execute($borrarDepositos);
		
		if($dbAcs->completetrans()){
			//print "ok";
		}
		else{
			$dbAcs->rollbacktrans();
