...
//==============================================================
//==============================================================
if ($_REQUEST['html']) { echo '<html><head><style>'.file_get_contents('mpdfstyletables.css').'</style></head><body><div style="width:50%;">'.$html.'</div></body></html>'; exit; }
if ($_REQUEST['source']) { 
	$file = __FILE__;
...
$mpdf=new mPDF('en-GB','A4','','',32,25,27,25,16,13); 
	...
	$mpdf->jSpacing = 'C';
	$test = "<h5>Character spacing</h5><p>Maecenas feugiat pede vel risus. Nulla et lectus eleifend <i>verylongwordthatwontsplitanywhere</i> neque sit amet erat</p>";
	$mpdf->WriteHTML($test,2);

	$mpdf->jSpacing = 'W';
	$test = "<h5>Word spacing</h5><p>Maecenas feugiat pede vel risus. Nulla et lectus eleifend <i>verylongwordthatwontsplitanywhere</i> neque sit amet erat</p>";
	$mpdf->WriteHTML($test,2);

	$mpdf->jSpacing = '';
	$test = "<h5>Mixed Character and Word spacing</h5><p>Maecenas feugiat pede vel risus. Nulla et lectus eleifend <i>verylongwordthatwontsplitanywhere</i> neque sit amet erat</p>";
	$mpdf->WriteHTML($test,2);


	// ORPHANS
	$test = "<h4>Orphans</h4><p>Punctuation marks should not be split when the last word on a line is in &lt;tags&gt; eg <acronym>NATO</acronym>. The punctuation should go to the next line with the word if necessary.</p>";
	$mpdf->WriteHTML($test,2);
	$test = "<p>Punctuation marks should not be split when the last word on a line is in &lt;tags&gt; e.g. <acronym>UNESCO</acronym>. The punctuation should go to the next line with the word if necessary.</p>";
	$mpdf->WriteHTML($test,2);
	$test = "<p>Single marks .,?!;:\xe2\x80\x9e\xe2\x80\x9d should be preserved when last word on a line is in &lt;tags&gt; e.g. <acronym>NATO</acronym>? All of these are automatically protected in mPDF.</p>";
	$mpdf->WriteHTML($test,2);

	$mpdf->orphansAllowed = 0;
	$test = "<p>Similarly, sub and super texts should not be split when justifying text such as references<sup>23</sup>. (Note that this next sentence has the orphansAllowed set to zero.) Altering the value of \$mpdf->orphansAllowed will determine how many extra characters can be preserved on a line; the next line allows 3 (default value 5):</p>";
	$mpdf->WriteHTML($test,2);

	$mpdf->orphansAllowed = 3;
	$test = "<p>Similarly, sub and super texts should not be split when justifying text such as references<sup>23</sup>. The references should go to the next line with the word if necessary.</p>";
	$mpdf->WriteHTML($test,2);
