...
var $pregUHCchars = "\x{1100}-\x{11FF}\x{3130}-\x{318F}\x{AC00}-\x{D7AF}";
var $pregSJISchars = "\x{3040}-\x{309F}\x{30A0}-\x{30FF}\x{31F0}-\x{31FF}";

// Chars which distinguish CJK but not between different
var $pregCJKchars = "\x{2E80}-\x{A4CF}\x{A800}-\x{D7AF}\x{F900}-\x{FAFF}\x{FF00}-\x{FFEF}\x{20100}-\x{201FF}\x{2F800}-\x{2FA1F}";
...
function AddPage($orientation='',$condition='', $resetpagenum='', $pagenumstyle='', $suppress='',$mgl='',$mgr='',$mgt='',$mgb='',$mgh='',$mgf='',$ohname='',$ehname='',$ofname='',$efname='',$ohvalue=0,$ehvalue=0,$ofvalue=0,$efvalue=0)
{
	...
	// Paint Div Border if necessary
   	...
   	if (!$this->ColActive && $this->blklvl > 0) {
		...
		// This needs to call PaintDivBorder (excluded in mPDF 1.3) but not for the current (new) block level
	...
	if ($this->page_box['changed']) {
		list($orientation,$mgl,$mgr,$mgt,$mgb,$mgh,$mgf,$mgtfp,$mgbfp,$ohname,$ehname,$ofname,$efname) = $this->SetPagedMediaCSS();
   ...
   $newblock = $this->flowingBlockAttr[ 'newblock' ];
	...
	if ($is_table) { 
		...
		if($this->y+$check_h > $this->PageBreakTrigger and !$this->InFooter and $this->AcceptPageBreak()) {
			...
			// mPDF 2.5 - Bug fix - cMarginL and R reset in AddPage
		...
		// IMAGE
		   if ($objattr['type'] == 'image') {
			...
			if ($objattr['itype']=='wmf') { 
				...
				$outstring = sprintf("q %.2f 0 0 %.2f %.2f %.2f cm /I%d Do Q",$objattr['INNER-WIDTH'] *$this->k,$objattr['INNER-HEIGHT'] *$this->k,$objattr['INNER-X'] *$this->k,($this->h-($objattr['INNER-Y'] +$objattr['INNER-HEIGHT'] ))*$this->k,$objattr['ID'] );
			}
			$this->_out($outstring);
			// LINK
   ...
   if ($is_table) { 
	...
	$cpaddingadjustR = 0;
     ...
     //OBJECTS - IMAGES & FORM Elements (NB has already skipped line/page if required - in printbuffer)
      if ($s{0} == "\xbb" and $s{1} == "\xa4" and $s{2} == "\xac") { //identifier has been identified!
   ...
   for ( $i = 0; $i < $tmp; $i++ )  {
	...
	if (( $contentWidth + $cw > $maxWidth - $WidthCorrection - (($this->cMarginL+$this->cMarginR)*$this->k) - ($paddingL+$paddingR +(($fpaddingL + $fpaddingR) * $this->k) ) +  0.001))  {// 0.001 is to correct for deviations converting mm=>pts
		...
		// cut off and save any partial words at the end of the string
			...
			$success = false;
			if (($this->is_MB && preg_match("/\xc2\xad/",$currWord)) || (!$this->is_MB && preg_match("/".chr(173)."/",$currWord)) ) {
...
function Output($name='',$dest='')
{
	...
	if (ob_get_contents()) { echo "<p>Output has already been sent from the script - PDF file generation aborted.</p>"; exit; }
...
function _putpages()
{
	...
	if(!empty($this->aliasNbPgGp))
	{
		...
		for($n=1;$n<=$nb;$n++) {
			if ($this->is_MB) { $r = $this->UTF8ToUTF16BE($this->docPageNumTotal($n), false); }
			else { $r = $this->docPageNumTotal($n); }
			// mPDF 2.4
			if (preg_match_all('/{mpdfheadernbpggp (C|R) ff=(\S+) fs=(\S+) fz=(.*?)}/',$this->pages[$n],$m)) {
				for($hi=0;$hi<count($m[0]);$hi++) {
					...
					$x2 = $this->GetStringWidth($this->docPageNumTotal($n));
					$xadj = $x1 - $x2;
					if ($pos=='C') { $xadj /= 2; }
					$rep = sprintf(' q 1 0 0 1 %.3f 0 cm ', $xadj*$this->k); 
					$this->pages[$n] = str_replace($m[0][$hi], $rep, $this->pages[$n]);
		...
		if(isset($this->PageLinks[$n]) || isset($this->PageAnnots[$n])) {
			...
			if(isset($this->PageLinks[$n])) {
			   foreach($this->PageLinks[$n] as $key => $pl) {
				...
				if(is_string($pl[4])) {
					...
					$annot.=sprintf(' /Dest [%d 0 R /XYZ 0 %.2f null]>>',1+2*$l[0],$h-$l[1]*$this->k);
				}
				$this->_out($annot);
				$this->_out('endobj');
			   }
			...
			if(isset($this->PageAnnots[$n])) {
			   foreach ($this->PageAnnots[$n] as $key => $pl) {
				...
				$a = $x * $this->k;
				$b = ($this->h - $pl['y'])  * $this->k;
				$rect = sprintf('%.2f %.2f %.2f %.2f', $a, $b-$h, $a+$w, $b);
...
function _parsephp($file) {
	...
	if (!$accessfile) { 
		return -1; 
		$this->Error("PHP parser: unable to open file : ".$file); 
	}
	$c = curl_init();
...
function _parsejpg($file) {
	...
	if (!$accessfile) { 
		return -1; 
		$this->Error("JPG parser: unable to open file : ".$file); 
	}
...
function _parsepng($file) {
	...
	if (!$accessfile) { 
		return -1; 
		$this->Error("PNG parser: unable to open file : ".$file); 
	}
	...
	if (!$f) { 
		if ($isTemp) { unlink($accessfile); } 
		return -1; 
		$this->Error("PNG parser: unable to open file - access-name: $accessfile - filename: ".$file); 
	}
...
function _parsegif($file) { //EDITEI - GIF support is now included 
	...
	if (!$gif->loadFile($accessfile, 0)) { 
		if ($isTemp) { unlink($accessfile); } 
		return -1; 
		$this->Error("GIF parser: unable to open file - access-name: $accessfile - filename: ".$file); 
	}
...
// Added mPDF 2.0
function docPageNumTotal($num = 0) {
	if ($num < 1) { $num = $this->page; }
	$type = '1';	// set default decimal
	$ppgstart = 1;
	...
	foreach($this->PageNumSubstitutions AS $psarr) {
		if ($num >= $psarr['from']) {
			if ($psarr['reset']) { $ppgstart = $psarr['from']; }
			if ($psarr['type']) { $type = $psarr['type']; }
			if (strtoupper($psarr['suppress'])=='ON' || $psarr['suppress']==1) { $suppress = 1; }
			else if (strtoupper($psarr['suppress'])=='OFF') { $suppress = 0; }
		}
...
function Header($content='') {
  ...
  if(count($h)) {
	...
	if ($this->forcePortraitHeaders && $this->CurOrientation=='L' && $this->CurOrientation!=$this->DefOrientation) {
		$this->_out(sprintf('q 0 -1 1 0 0 %.2f cm ',($this->h*$this->k)));
		$yadj = $this->w - $this->h;
	...
	foreach(array('L','C','R') AS $pos) {
	  if ($h[$side][$pos]['content']) {
		...
		if ($h[$side][$pos]['font-size']) { $hfsz = $h[$side][$pos]['font-size']; }
		else { $hfsz = $this->default_font_size; }	// pts
		$maxfontheight = max($maxfontheight,$hfsz);
   ...
   for ($i=0; $i<count($content); $i++) {
	...
	// TABLE BORDER - if separate
 	if ($table['borders_separate'] && $table['border']) { 
			...
			$this->_tableRect($tbx, $tby, $tbw, $tbh, $tab_bord , $table['border_details'], false, $table['borders_separate']);
	}
...
function SetHTMLHeader($Hhtml='',$OE='',$write=false) {
	...
	for($i = 1;$i<=5;$i++) {
		...
		if ($OE == 'E') {// EVEN
			if ($i==1) $this->saveHTMLHeaderE_N = $this->headerbuffer;
			else if ($i==2) $this->saveHTMLHeaderE_NN = $this->headerbuffer;
			else if ($i==3) $this->saveHTMLHeaderE_NNN = $this->headerbuffer;
			else if ($i==4) $this->saveHTMLHeaderE_NNNN = $this->headerbuffer;
			else if ($i==5) $this->saveHTMLHeaderE_NNNNN = $this->headerbuffer;
		}
		else {	// ODD	// OR NOT MIRRORING MARGINS/FOOTERS = DEFAULT
			if ($i==1) $this->saveHTMLHeader_N = $this->headerbuffer;
			else if ($i==2) $this->saveHTMLHeader_NN = $this->headerbuffer;
			else if ($i==3) $this->saveHTMLHeader_NNN = $this->headerbuffer;
			else if ($i==4) $this->saveHTMLHeader_NNNN = $this->headerbuffer;
			else if ($i==5) $this->saveHTMLHeader_NNNNN = $this->headerbuffer;
		}
	}
	$this->ResetMargins();
	$this->pgwidth = $this->w - $this->lMargin - $this->rMargin;
...
function SetHTMLFooter($Fhtml='',$OE='') {
	...
	for($i = 1;$i<=5;$i++) {
		...
		if ($OE == 'E') {// EVEN
			...
			if ($i==1) $this->saveHTMLFooterE_N = $this->headerbuffer;
			else if ($i==2) $this->saveHTMLFooterE_NN = $this->headerbuffer;
			else if ($i==3) $this->saveHTMLFooterE_NNN = $this->headerbuffer;
			else if ($i==4) $this->saveHTMLFooterE_NNNN = $this->headerbuffer;
			else if ($i==5) $this->saveHTMLFooterE_NNNNN = $this->headerbuffer;
		}
		else {	// ODD	// OR NOT MIRRORING MARGINS/FOOTERS = DEFAULT
			$this->saveHTMLFooter_height = $this->y - $top_y;
			if ($i==1) $this->saveHTMLFooter_N = $this->headerbuffer;
			else if ($i==2) $this->saveHTMLFooter_NN = $this->headerbuffer;
			else if ($i==3) $this->saveHTMLFooter_NNN = $this->headerbuffer;
			else if ($i==4) $this->saveHTMLFooter_NNNN = $this->headerbuffer;
			else if ($i==5) $this->saveHTMLFooter_NNNNN = $this->headerbuffer;
		}
	}
...
function writeHTMLHeaders() {
	...
	$os = '';
	if ($this->forcePortraitHeaders && $this->CurOrientation=='L' && $this->CurOrientation!=$this->DefOrientation) {
		$os = sprintf('q 0 -1 1 0 0 %.2f cm ',($this->h*$this->k));
	}
	if ($this->is_MB) {
...
function WriteHTML($html,$sub=0,$init=true,$close=true) {
	...
	if ($sub < 2) { 
		...
		if ($this->state==0 && $pm['ISSET']) {
			...
			list($pborientation,$pbmgl,$pbmgr,$pbmgt,$pbmgb,$pbmgh,$pbmgf,$pbmgtfp,$pbmgbfp,$ohname,$ehname,$ofname,$efname) = $this->SetPagedMediaCSS();
...
function SetPagedMediaCSS() {
		...
		return array($orientation,$mgl,$mgr,$mgt,$mgb,$mgh,$mgf,$mgtfp,$mgbfp,$ohname,$ehname,$ofname,$efname);
}
...
function OpenTag($tag,$attr)
{
  ...
  switch($tag){
	...
	if ($toc_id && $this->y == $this->tMargin && (!$this->useOddEven ||($this->useOddEven && $this->page % 2==1))) { 
	...
	if ($attr['CONTENT']) {
		$this->Reference(htmlspecialchars_decode($attr['CONTENT'],ENT_QUOTES));
	}
	break;
	...
	break;
     ...
     // mPDF 2.2 Annotations
     case 'ANNOTATION': //added custom-tag
	if ($attr['CONTENT']) {
		$objattr = array();
		// mPDF 2.3
	...
	if ($this->tableLevel) {
	   // If already something in the Cell
	   if (((isset($this->cell[$this->row][$this->col]['maxs']) && $this->cell[$this->row][$this->col]['maxs'] > 0 ) || $this->cell[$this->row][$this->col]['s'] > 0) && !$this->nestedtablejustfinished ) {
		$this->cell[$this->row][$this->col]['textbuffer'][] = array("\n",$this->HREF,$this->currentfontstyle,$this->colorarray,$this->currentfontfamily,$this->SUP,$this->SUB,''/*internal link*/,$this->strike,$this->outlineparam,$this->spanbgcolorarray,$this->currentfontsize);
		$this->cell[$this->row][$this->col]['text'][] = "\n";
		if (!isset($this->cell[$this->row][$this->col]['maxs'])) {
			$this->cell[$this->row][$this->col]['maxs'] = $this->cell[$this->row][$this->col]['s'];
	...
	// Added mPDF 1.1 keeping block together on one page
	// mPDF 2.0 Disallowed if using Active Forms
		...
		while(!$imageset) {
			...
			if(!isset($this->images[$srcpath])) {
				...
				if(!$pos) { 
					$found_img = false; 
					continue; 
					//$this->Error('Image file has no extension and no type was specified: '.$srcpath);
				}
				// mPDF 2.5
				...
				elseif($itype=='wmf')  { $found_img = false; continue; }
					...
					if(method_exists($this,$mtd)) { $info=$this->$mtd($srcpath); }
						...
						$info = -1; 
						//$this->Error('Unsupported image type: '.$itype);
					}
				}
				set_magic_quotes_runtime($mqr);
	...
	if ($maxrowheight > $fullpage) { 
		...
		if ($tableheight > $remainingpage) { 
			...
			if (($this->shrin_k * $this->tbsqrt($tableheight / $remainingpage, 1)) <= $this->shrink_this_table_to_fit) {
				...
				$this->kwt_moved = true; 
				$this->tbrot_maxw = $this->h - ($this->y + $this->bMargin + 5) - $this->kwt_height;
				$recalculate = (1 / $this->shrin_k); 	// undo any shrink
			}
		}
	}
...
// $blockstate = 3;	// Top & bottom margins/padding
	...
	// SPECIAL CONTENT - IMAGES & FORM OBJECTS
      ...
      if (isset($vetor[0]) and $vetor[0]{0} == "\xbb" and $vetor[0]{1} == "\xa4" and $vetor[0]{2} == "\xac") { //identifier has been identified!
...
function TableWordWrap($maxwidth, $forcewrap = 0, $textbuffer = '', $returnarray=false)
{
   ...
   foreach ($textbuffer as $cctr=>$chunk) {
		...
		//IMAGE
      	if ($line{0} == "\xbb" and $line{1} == "\xa4" and $line{2} == "\xac") { //identifier has been identified!
	...
	foreach ($words as $word) {
		...
		// Word does not fit on line...
			...
			if ($alloworphans) {
				...
				// mPDF 2.5 Soft hyphens
				if (($this->is_MB && preg_match("/\xc2\xad/",$word)) || (!$this->is_MB && preg_match("/".chr(173)."/",$word)) ) {
...
function TableCheckMinWidth(&$text, $maxwidth, $forcewrap = 0, $textbuffer = '')
{
    ...
    foreach ($textbuffer as $chunk) {
		...
		// IMAGES & FORM ELEMENTS
	      if ($line{0} == "\xbb" and $line{1} == "\xa4" and $line{2} == "\xac") { //inline object - FORM element or IMAGE!
			$sccontent = split("\xbb\xa4\xac",$line,2);
			$sccontent = split(",",$sccontent[1],2);
			foreach($sccontent as $scvalue) {
				$scvalue = split("=",$scvalue,2);
...
function _tableRect($x, $y, $w, $h, $bord=-1, $details=array(), $buffer=false, $bSeparate=false, $cort='cell'){
	// mPDF 2.1
	...
	if ($bord==-1) { $this->Rect($x, $y, $w, $h); }
	   ...
	   if (!$bSeparate && $buffer) {
		...
		for($p=0;$p<strlen($priority);$p++) {
			...
			$coldom = (1-(($details[$side]['c']['R']+$details[$side]['c']['G']+$details[$side]['c']['B'])/765)); // 1 black - 0 white
			if ($coldom) { $dom += 2; }
...
/*
			// Set Priority (mPDF 2.0) to write so overlapping is minimized
			// ? unnecessary as borderbuffer now used
			if (!$table['borders_separate']) {
			   if ($i == 0 && $j==0) {	// Top Left
				$cell['border_details']['p'] = 'RBLT';
			   }
			   else if ($i == 0 && ($j == ($numcols-1) || ($j+$cell['colspan']) == ($numcols) ) ) {	// Top Right
				$cell['border_details']['p'] = 'BLTR';
			   }
 			   else if ($i == 0) {		// Top
				$cell['border_details']['p'] = 'RBLT';
			   }
			   else if ($j == 0 && ($i == ($numrows-1) || ($i+$cell['rowspan']) == ($numrows) )) {	// Bottom Left
				$cell['border_details']['p'] = 'TRBL';
			   }
			   else if (($j == ($numcols-1) || ($j+$cell['colspan']) == ($numcols) )  && ($i == ($numrows-1) || ($i+$cell['rowspan']) == ($numrows) )) {	// Bottom Right
				$cell['border_details']['p'] = 'LTRB';
			   }
			   else if ($i == ($numrows-1) || ($i+$cell['rowspan']) == ($numrows) ) {	// Bottom
				$cell['border_details']['p'] = 'LTRB';
			   }
			   else if ($j == 0) {		// Left
				$cell['border_details']['p'] = 'TRBL';
			   }
			   else if ($j == ($numcols-1) || ($j+$cell['colspan']) == ($numcols) ) {	// Right
				$cell['border_details']['p'] = 'BLTR';
			   }
			}
*/
			...
			//if (!$table['borders_separate'] && is_array($cell['border_details']) && (!$this->usetableheader || $i>0) ) {
			if (!$table['borders_separate'] && (!$this->usetableheader || $i>0) ) {
		...
		if (isset($cells[$i][$j]) && $cells[$i][$j]) {
			...
			if ((($y + $maxrowheight + $extra ) > $pagetrigger) && ($y0 >0 || $x0 > 0) && !$this->InFooter) {
				if (!$skippage) {
					...
					if ($this->AcceptPageBreak()) {
						...
						// mPDF 2.0 - To develop - This adds a blank background fill to cover border-spacing or 
						...
						if($i>0 && !$this->table_rotate && $level==1 && !$this->ColActive) {
							...
							else { $adv = $cell['border_details']['B']['w']/2; }

							if ($table['border_details']['L']['w']/2 > $adv) { $adv = $table['border_details']['L']['w']/2; }
							if ($table['border_details']['R']['w']/2 > $adv) { $adv = $table['border_details']['R']['w']/2; }

			  				$this->SetFillColor(255);
							$this->Rect($this->lMargin, $this->y, $this->pgwidth, $adv, 'F');
							// Reset current block fill
							$bcor = $this->blk[$this->blklvl]['bgcolorarray'];
			...
			// TABLE BORDER - if separate
 			if ($table['borders_separate'] && $table['border']) { 
			   ...
			   $this->_tableRect($tbx, $tby, $tbw, $tbh, $tab_bord , $table['border_details'], false, $table['borders_separate'], 'table');
			}
			...
			if ($this->usetableheader and ($i == 0  || $table['is_thead'][$i]) && $level==1) {
				$tableheader[$i][$j]['x'] = $x;
				$tableheader[$i][$j]['y'] = $y;
				$tableheader[$i][$j]['h'] = $h;
				$tableheader[$i][$j]['w'] = $w;
			...
			if ($bord || $bord_det) { 
				...
				// mPDF 2.0 - To develop - This adds a blank background fill to cover half of top border-spacing or 
				// extra half border width in case page was broken in middle
				// doesn't work for !$table['borders_separate'] (collapsed) as they are output later in printcellbuffer
				// would need to save it in buffer somehow
				if($i>0 && !$this->table_rotate && $level==1 && $newpagestarted && !$this->usetableheader) {
					if ($table['borders_separate']) { $adv = $table['border_spacing_V']/2; }
					else { $adv = $cell['border_details']['B']['w']/2; }
	  				$this->SetFillColor(255);
					$this->Rect($this->lMargin, $y-$adv, $this->pgwidth, $adv, 'F');
					// Reset current block fill
					$bcor = $this->blk[$this->blklvl]['bgcolorarray'];
					if ($bcor ) $this->SetFillColor($bcor['R'],$bcor['G'],$bcor['B']);
				}
...
// FROM class PDF_TOC
	...
	function TOCpagebreak($tocfont='', $tocfontsize='', $tocindent='', $TOCusePaging='', $TOCuseLinking='', $toc_orientation='', $toc_mgl='',$toc_mgr='',$toc_mgt='',$toc_mgb='',$toc_mgh='',$toc_mgf='',$toc_ohname='',$toc_ehname='',$toc_ofname='',$toc_efname='',$toc_ohvalue=0,$toc_ehvalue=0,$toc_ofvalue=0,$toc_efvalue=0, $toc_preHTML='', $toc_postHTML='', $toc_bookmarkText='', $resetpagenum='', $pagenumstyle='', $suppress='', $orientation='', $mgl='',$mgr='',$mgt='',$mgb='',$mgh='',$mgf='',$ohname='',$ehname='',$ofname='',$efname='',$ohvalue=0,$ehvalue=0,$ofvalue=0,$efvalue=0, $toc_id=0) {
		...
		// mPDF 2.2 Use AddPage not AddPages
		// mPDF 2.3
		if ($toc_id && $this->y == $this->tMargin && (!$this->useOddEven ||($this->useOddEven && $this->page % 2==1))) { $dummy=1; }
		else {
		...
		if (!$TOCusePaging && $TOCusePaging !== false && $TOCusePaging !== 0 && $TOCusePaging !== "0") { $TOCusePaging = true; }
		else { $TOCusePaging = false; }
	...
	function TOC_Entry($txt, $level=0, $toc_id=0) {
		...
		// Edited mPDF 1.1 Keep Block together
		if ($this->keep_block_together) {
			$this->_kttoc[]=array('t'=>$txt,'l'=>$level,'p'=>$this->docPageNum(), 'op'=>$this->page, 'link'=>$linkn, 'toc_id'=>$toc_id);
		}
		else {
			$this->_toc[]=array('t'=>$txt,'l'=>$level,'p'=>$this->docPageNum(), 'link'=>$linkn, 'toc_id'=>$toc_id);
		}
	}
...
function insertTOC() {
	...
	for ($toci = 0; $toci<$notocs; $toci++) {
		...
		if (!$this->TOCorientation) { $this->TOCorientation = $this->DefOrientation; }
		$this->AddPage($toc_orientation, "O", '', '', "on", $toc_mgl, $toc_mgr, $toc_mgt, $toc_mgb, $toc_mgh, $toc_mgf, $toc_ohname, $toc_ehname, $toc_ofname, $toc_efname, $toc_ohvalue, $toc_ehvalue, $toc_ofvalue, $toc_efvalue);
		 ...
		 if ($t['toc_id']==='_mpdf_all' || $t['toc_id']===$toc_id ) {
		   ...
		   if ($this->directionality == 'rtl') {
			...
			if ($TOCusePaging) {
				...
				$this->Cell($PageCellSize,$this->FontSize+$lineheightcorr,$t['p'],0,0,'R',0,$t['link']);
			...
			$this->Cell($strsize+2,$this->FontSize+$lineheightcorr,$str,0,1,'L',0,$t['link']);

		   }
		   else {
			if($level>0) { $this->Cell($level*$tocindent,$this->FontSize+$lineheightcorr); }
			$PageCellSize=$this->GetStringWidth($t['p'])+2;
			...
			if ($TOCusePaging) {
				// Text
				$this->Cell($strsize+2,$this->FontSize+$lineheightcorr,$str,0,0,'',0,$t['link']);
				...
				//Page number
				// Edited mPDF 2.0 change ToC paging
				$this->Cell($PageCellSize,$this->FontSize+$lineheightcorr,$t['p'],0,1,'R',0,$t['link']);
			}
			else {
				// Text only
				$this->Cell($strsize+2,$this->FontSize+$lineheightcorr,$str,0,1,'',0,$t['link']);	// forces new line
			}
		...
		for($i = $tocstart;$i <= $n;$i++)
			$last[]=$this->pages[$i];
		//move pages
		for($i=$tocstart - 1;$i>=$toc_page-1;$i--) {
			$this->pages[$i+$n_toc]=$this->pages[$i];
		}
		//Put toc pages at insert point
		for($i = 0;$i < $n_toc;$i++) {
			$this->pages[$toc_page + $i]=$last[$i];
		}
		...
		if (count($this->PageLinks)) {
		   ...
		   foreach($this->PageLinks as $i=>$o) {
			...
			if($i>=$tocstart) {
				$newarr[($toc_page + ($i - $tocstart))] = $this->PageLinks[$i];
			}
			else if($i>=$toc_page ) {
		...
		// mPDF 2.0 OrientationChanges
		if (count($this->OrientationChanges)) {
			if (strtoupper($toc_orientation{0}) != $this->DefOrientation) { array_pop($this->OrientationChanges); }
			//update pagenumbers
			$newarr = array();
			foreach($this->OrientationChanges AS $p=>$v) {
			  if ($p < $this->TOCmark) {
				$newarr[$p] = $this->OrientationChanges[$p];
				unset($this->OrientationChanges[$p]);
			  }
			}
			for ($i=0; $i<$n_toc; $i++) {
				if (strtoupper($toc_orientation{0}) != $this->DefOrientation) {
					$newarr[$toc_page+$i] = true;
				}
			}
			foreach($this->OrientationChanges AS $p=>$v) {
			  $newarr[$p+$n_toc] = true;
		...
		if (count($this->internallink)) {
		   foreach($this->internallink as $key=>$o) {
			// Edited mPDF 2.0 - move links in ToC itself
			if($o['PAGE']>=$tocstart) {
				$this->internallink[$key]['PAGE'] -= ($tocstart + $n_toc - $toc_page);
			}
			else if($o['PAGE']>=$toc_page) {
		...
		if (count($this->links)) {
		   foreach($this->links as $key=>$o) {
			// Edited mPDF 2.0 - move links in ToC itself
			if($o[0]>=$tocstart) {
				$this->links[$key][0] -= ($tocstart + $n_toc - $toc_page);
			}
			if($o[0]>=$toc_page) {
		...
		// mPDF 2.0 Update PageNumSubstitutions
		if (count($this->PageNumSubstitutions)) {
			$tarr = array_pop($this->PageNumSubstitutions);
			$tarr['from'] = $toc_page;
			//update pagenumbers
			$newarr = array();
			foreach($this->PageNumSubstitutions AS $k=>$v) {
			  if ($this->PageNumSubstitutions[$k]['from']<$toc_page) {
				$newarr[] = $this->PageNumSubstitutions[$k];
				unset($this->PageNumSubstitutions[$k]);
			  }
			  else if ($this->PageNumSubstitutions[$k]['from']>=$toc_page) {
				$this->PageNumSubstitutions[$k]['from'] += $n_toc;
			  }
			}
			$newarr[] = $tarr;
			$newarr = array_merge($newarr,$this->PageNumSubstitutions);
			$this->PageNumSubstitutions = $newarr;
		}
	}
}
...
function CreateReference($NbCol=1, $reffontsize='', $linespacing=1, $offset=3, $usedivletters=1, $divlettfontsize='', $gap=5, $reffont='',$divlettfont='') {
	$this->CreateIndex($NbCol, $reffontsize, $linespacing, $offset, $usedivletters, $divlettfontsize, $gap, $reffont, $divlettfont);
}
// mPDF 2.2
function CreateIndex($NbCol=1, $reffontsize='', $linespacing=1, $offset=3, $usedivletters=1, $divlettfontsize='', $gap=5, $reffont='',$divlettfont='') {
	...
	for ($i=0;$i<$size;$i++){
		...
		if ($usedivletters) {
		   ...
		   if ($lett != $last_lett) {
			...
			$this->SetStyle('B',true);
			$this->Cell($colWidth,$this->FontSize,$lett,0,1,$align);
			$this->SetFont($reffont,'',$reffontsize);
		   }
		}
		...
		if ($strsize > $colWidth - 2) {
			...
			if (!$str1) { 	// Failsafe if goes wrong
				$this->MultiCell($colWidth,$this->FontSize*$linespacing,'FAIL'.$str,0,'L',0,'','ltr',true);
			}
			else {
				// DIRECTIONALITY RTL
				$this->magic_reverse_dir($str1);	// MultiCell does magic reverse - Cell doesn't
				$this->Cell($colWidth,$this->FontSize*$linespacing,$str1,0,1,$align);
				if ($this->directionality == 'rtl') {
				...
				$this->MultiCell($colWidth-$offset,$this->FontSize*$linespacing,$str2,0,$align,0,'','ltr',true);
				$this->SetX($this->lMargin);
				$this->x=$this->lMargin;
			}
		}
			...
			$this->Cell($colWidth,$this->FontSize*$linespacing,$str,0,1,$align);	// was strsize - changed for RTL
		}
...
function printdivbuffer() {
	...
	if ($this->ColActive || !$this->keep_block_together || $this->blk[$this->blklvl]['startpage'] == $this->page || ($this->page - $this->blk[$this->blklvl]['startpage']) > 1 || ($height[$p1]+$height[$p2]) > $this->h) { 
	   ...
	   // Adjust Reference (index) - uses document page number
	   foreach($this->ktReference AS $v) {
		// Edited mPDF 2.0
		if ($v['op'] == $p1) { 
			//$v['p'] += 1; 	// doesn't work if Roman numerals etc.
			$v['p'] = $this->docPageNum($p2);
		}
		$this->Reference[]=array('t'=>$v['t'],'p'=>$v['p']);
	   }
...
function spanChinese($text) {
	$o = '';
	if (!mb_ereg('\?', mb_convert_encoding($text, 'CP936', 'UTF-8'))) {
		$o = "<span style='font-family:GB'>".$text."</span>";
	}
	else if (!mb_ereg('\?', mb_convert_encoding($text, 'BIG-5', 'UTF-8'))) {
		$o = "<span style='font-family:BIG5'>".$text."</span>";
	}
	else {
	  $chars = preg_split('//u', $text);
	  foreach ($chars AS $char) {
	    if (mb_convert_encoding($char, 'CP936', 'UTF-8') != '?') {
		$o .= "<span style='font-family:GB'>".$text."</span>";
	    }
	    else if (mb_convert_encoding($char, 'BIG-5', 'UTF-8') != '?') {
		$o .= "<span style='font-family:BIG5'>".$text."</span>";
	    }
	  }
	}
...
function replaceCJK($str) {
	if (preg_match("/[".$this->pregUHCchars."]/u", $str)) { 
		return '<span lang="ko">' . $str .'</span>';
	}
	else if (preg_match("/[".$this->pregSJISchars."]/u", $str)) { 
		return '<span lang="ja">' . $str .'</span>';
	}
	else{ 
		return '<span lang="zh">' . $str .'</span>';
	}
	return $str;
}
...
function replaceArabic($str) {
	if (!$this->rtlAsArabicFarsi && preg_match("/[".$this->pregNonARABICchars ."]/u", $str) ) {
		// PASHTO, SINDHI, URDU
		return '<span lang="ur">'.$str.'</span>';	
	}
	else {
		// ARABIC, PERSIAN
		return '<span lang="ar">'.$str.'</span>';	
	}
	return $str;
}
	...
	foreach($a as $i => $e) {
	   if($i%2==0) {
		...
		if ($this->autoFontGroups & AUTOFONT_THAIVIET) {
			// THAI
			$e = preg_replace("/([\x{0E00}-\x{0E7F}".$extra."]*[\x{0E00}-\x{0E7F}][\x{0E00}-\x{0E7F}".$extra."]*)/u", '<span lang="th">\\1</span>', $e);
			// Vietnamese
			$e = preg_replace("/([".$this->pregVIETchars .$this->pregVIETPluschars ."]*[".$this->pregVIETchars ."][".$this->pregVIETchars .$this->pregVIETPluschars ."]*)/u", '<span lang="vi">\\1</span>', $e);	
		}

		if ($this->autoFontGroups & AUTOFONT_RTL) {
			// HEBREW
			$e = preg_replace("/([".$this->pregHEBchars .$extra."]*[".$this->pregHEBchars ."][".$this->pregHEBchars .$extra."]*)/u", '<span lang="he">\\1</span>', $e); 
			// All Arabic
		...
		if ($this->autoFontGroups & AUTOFONT_INDIC) {
			// Bengali
			$e = preg_replace("/([\x{0980}-\x{09FF}".$extra."]*[\x{0980}-\x{09FF}][\x{0980}-\x{09FF}".$extra."]*)/u", '<span lang="bn">\\1</span>', $e);	
			// Devanagari (= script for Hindi, Nepali + Sindhi)
			$e = preg_replace("/([\x{0900}-\x{097F}".$extra."]*[\x{0900}-\x{097F}][\x{0900}-\x{097F}".$extra."]*)/u", '<span lang="hi">\\1</span>', $e);	
			// Gujarati
			$e = preg_replace("/([\x{0A80}-\x{0AFF}".$extra."]*[\x{0A80}-\x{0AFF}][\x{0A80}-\x{0AFF}".$extra."]*)/u", '<span lang="gu">\\1</span>', $e);	
			// Malayalam
			$e = preg_replace("/([\x{0D00}-\x{0D7F}".$extra."]*[\x{0D00}-\x{0D7F}][\x{0D00}-\x{0D7F}".$extra."]*)/u", '<span lang="ml">\\1</span>', $e);	

			// Indic fonts with limited coverage of ASCII
			// Kannada
			$e = preg_replace("/([\x{0C80}-\x{0CFF}".$this->pregASCIILchars."]*[\x{0C80}-\x{0CFF}][\x{0C80}-\x{0CFF}".$this->pregASCIILchars."]*)/u", '<span lang="kn">\\1</span>', $e);	
			// Oriya
			$e = preg_replace("/([\x{0B00}-\x{0B7F}".$this->pregASCIILchars."]*[\x{0B00}-\x{0B7F}][\x{0B00}-\x{0B7F}".$this->pregASCIILchars."]*)/u", '<span lang="or">\\1</span>', $e);	
			// Punjabi ?= Gurmuhki 
			$e = preg_replace("/([\x{0A00}-\x{0A7F}".$this->pregASCIILchars."]*[\x{0A00}-\x{0A7F}][\x{0A00}-\x{0A7F}".$this->pregASCIILchars."]*)/u", '<span lang="pa">\\1</span>', $e);	
			// Tamil
			$e = preg_replace("/([\x{0B80}-\x{0BFF}".$this->pregASCIILchars."]*[\x{0B80}-\x{0BFF}][\x{0B80}-\x{0BFF}".$this->pregASCIILchars."]*)/u", '<span lang="ta">\\1</span>', $e);	
			// Telugu
			$e = preg_replace("/([\x{0C00}-\x{0C7F}".$this->pregASCIILchars."]*[\x{0C00}-\x{0C7F}][\x{0C00}-\x{0C7F}".$this->pregASCIILchars."]*)/u", '<span lang="te">\\1</span>', $e);	
		}
		$a[$i] = $e;
	   }
	   else {
