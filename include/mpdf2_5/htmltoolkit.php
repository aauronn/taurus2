...
function columnAdjustAdd($type,$k,$xadj,$yadj,$a,$b,$c=0,$d=0,$e=0,$f=0) {
   if ($type == 'Td') { 	// xpos,ypos
	$a += ($xadj * $k);
	$b -= ($yadj * $k);
	return 'BT '.sprintf('%.2f %.2f',$a,$b).' Td'; 
   }
   else if ($type == 're') { 	// xpos,ypos,width,height
	$a += ($xadj * $k);
	$b -= ($yadj * $k);
	return sprintf('%.2f %.2f %.2f %.2f',$a,$b,$c,$d).' re'; 
   }
   else if ($type == 'l') { 	// xpos,ypos,x2pos,y2pos
	$a += ($xadj * $k);
	$b -= ($yadj * $k);
	return sprintf('%.2f %.2f l',$a,$b); 
   }
   else if ($type == 'img') { 	// width,height,xpos,ypos
	$c += ($xadj * $k);
	$d -= ($yadj * $k);
	return sprintf('q %.2f 0 0 %.2f %.2f %.2f',$a,$b,$c,$d).' cm /I'; 
   }
...
function AdjustHTML($html,$directionality='ltr',$usepre=true, $tabSpaces=8) {
	...
	$html = mb_eregi_replace('<script.*?</script>','',$html);

	// mPDF 2.2
	//Remove special comments
	$html = mb_eregi_replace('<!--mpdf','',$html);
	$html = mb_eregi_replace('mpdf-->','',$html);

	//Remove comments from HTML (should not appear in the PDF file)
	$html = mb_eregi_replace('<!--.*?-->','',$html);

	$html = mb_ereg_replace('\f','',$html); //replace formfeed by nothing
	$html = mb_ereg_replace('\r','',$html); //replace carriage return by nothing

	// Well formed XHTML end tags
	$html = preg_replace('/<(br|hr)\/>/i',"<\\1 />",$html);	

	// Get rid of empty <thead></thead>
	$html = mb_eregi_replace('<thead>\s*</thead>','',$html);
	...
	$html = mb_eregi_replace('<br />\s*',"<br />",$html);
	...
	if ($usepre) //used to keep \n on content inside <pre> and inside <textarea>
 	{
		...
		$html = mb_ereg_replace('\t',' ',$html); //replace tabs by spaces
		...
		$html = mb_ereg_replace('/li>\s+</(u|o)l','/li></\\1l',$html);
		$html = mb_ereg_replace('/(u|o)l>\s+</li','/\\1l></li',$html);
		$html = mb_ereg_replace('/li>\s+</(u|o)l','/li></\\1l',$html);
		$html = mb_ereg_replace('/li>\s+<li','/li><li',$html);
		// mPDF 2.1 - allows <ol class="">  to match
		//$html = mb_ereg_replace('<(u|o)l>\s+','<\\1l>',$html);
		$html = preg_replace('/<(u|o)l([^>]*)>[ ]+/i','<\\1l\\2>',$html);
		// Added mPDF 2.1
		$html = mb_ereg_replace('\s+<(u|o)l','<\\1l',$html);
		...
		while($thereispre) //Recover <pre attributes>content</pre>
		{
			...
			$temp[2][$iterator] = mb_ereg_replace('&',"&amp;",$temp[2][$iterator]);
			$temp[2][$iterator] = mb_ereg_replace('<',"&lt;",$temp[2][$iterator]);

			$temp[2][$iterator] = mb_ereg_replace('\t',str_repeat(" ",$tabSpaces),$temp[2][$iterator]);

			$temp[2][$iterator] = mb_ereg_replace('\n',"<br />",$temp[2][$iterator]);
		...
		{
			$temp2[2][$iterator] = mb_ereg_replace('&',"&amp;",$temp2[2][$iterator]);
			$temp2[2][$iterator] = mb_ereg_replace('<',"&lt;",$temp2[2][$iterator]);
